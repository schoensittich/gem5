    ...
    }
{
    ...
    (*out_msg).m_Len = (0);
    ((*m_requestToMemory_ptr)).enqueue(out_msg, clockEdge(), cyclesToTicks(Cycles(m_to_mem_ctrl_latency)));
}
}
...
Directory_Controller::da_sendDMAAck(Directory_TBE*& m_tbe_ptr, Addr addr)
{
    ...
    #ifndef NDEBUG
    if (!((m_tbe_ptr != NULL))) {
        panic("Runtime Error at MESI_Two_Level-dir.sm:380: %s.\n", "assert failure");
    ...
    }
{
    ...
    #ifndef NDEBUG
    if (!((m_tbe_ptr != NULL))) {
        panic("Runtime Error at MESI_Two_Level-dir.sm:412: %s.\n", "assert failure");
