/**
 * DO NOT EDIT THIS FILE!
 * File automatically generated by
 *   build_tools/sim_object_param_struct_cc.py:273
 */

#include "pybind11/pybind11.h"
#include "pybind11/stl.h"

#include <type_traits>

#include "base/compiler.hh"
#include "params/X86SMBiosSMBiosStructure.hh"
#include "sim/init.hh"
#include "sim/sim_object.hh"

#include "arch/x86/bios/smbios.hh"

namespace py = pybind11;

namespace gem5
{

static void
module_init(py::module_ &m_internal)
{
py::module_ m = m_internal.def_submodule("param_X86SMBiosSMBiosStructure");
    py::class_<X86SMBiosSMBiosStructureParams, SimObjectParams, std::unique_ptr<X86SMBiosSMBiosStructureParams, py::nodelete>>(m, "X86SMBiosSMBiosStructureParams")
        ;

    py::class_<gem5::X86ISA::smbios::SMBiosStructure, gem5::SimObject, std::unique_ptr<gem5::X86ISA::smbios::SMBiosStructure, py::nodelete>>(m, "gem5_COLONS_X86ISA_COLONS_smbios_COLONS_SMBiosStructure")
        ;

}

static EmbeddedPyBind embed_obj("X86SMBiosSMBiosStructure", module_init, "SimObject");

} // namespace gem5
