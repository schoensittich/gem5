/**
 * DO NOT EDIT THIS FILE!
 * File automatically generated by
 *   build_tools/sim_object_param_struct_cc.py:273
 */

#include "pybind11/pybind11.h"
#include "pybind11/stl.h"

#include <type_traits>

#include "base/compiler.hh"
#include "params/X86IntelMPExtConfigEntry.hh"
#include "sim/init.hh"
#include "sim/sim_object.hh"

#include "arch/x86/bios/intelmp.hh"

namespace py = pybind11;

namespace gem5
{

static void
module_init(py::module_ &m_internal)
{
py::module_ m = m_internal.def_submodule("param_X86IntelMPExtConfigEntry");
    py::class_<X86IntelMPExtConfigEntryParams, SimObjectParams, std::unique_ptr<X86IntelMPExtConfigEntryParams, py::nodelete>>(m, "X86IntelMPExtConfigEntryParams")
        ;

    py::class_<gem5::X86ISA::intelmp::ExtConfigEntry, gem5::SimObject, std::unique_ptr<gem5::X86ISA::intelmp::ExtConfigEntry, py::nodelete>>(m, "gem5_COLONS_X86ISA_COLONS_intelmp_COLONS_ExtConfigEntry")
        ;

}

static EmbeddedPyBind embed_obj("X86IntelMPExtConfigEntry", module_init, "SimObject");

} // namespace gem5
